<template>
  <div class="log-form mar_tb2">
    <div class="log-form-block">
      ...
      <input v-model="logUser" id="logUser" type="text" class="log-inp fl" placeholder="请输入用户名">
      <p class="fl">{{userErrors.errorText}}</p>
    </div>
    <div class="log-form-block">
      <h2 class="fl tar">密码：</h2>
      <input v-model="logPass" id="logPass" type="password" class="log-inp fl" placeholder="请输入密码">
      ...
      $.get("./db.json",(data)=>{
         let i=0
          for(i=0;i<data.user.length;i++){
            if(data.user[i].userName===this.logUser && data.user[i].password===this.logPass){
              this.$emit('has-log',this.logUser)
            }
            else{
              this.errorText='部分选项未通过'
