<template>
  <div class="log-form mar_tb2">
    <div class="log-form-block">
      ...
      <input v-model="regUser" id="regUser" type="text" class="log-inp fl" placeholder="请输入用户名">
      <p class="fl">{{userErrors.errorText}}</p>
    </div>
    <div class="log-form-block">
      <h2 class="fl tar">密码：</h2>
      <input v-model="regPass" id="regPass" type="password" class="log-inp fl" placeholder="请输入密码">
...
export default {
  ...
  methods:{
    ...
    onReg(){
      this.errorText=''
      $.get("./db.json",(data)=>{
        this.$emit('reging',data,this.regUser,this.regPass)
      })
    }
  },
  computed:{
